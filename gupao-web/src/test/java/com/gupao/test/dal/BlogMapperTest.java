...
public class BlogMapperTest extends BaseTest {
    ...
    public void selectBlogPosts嵌套查询() throws InterruptedException {//one to many 嵌套查询
        ...
        System.out.println(resultMap.getName());  //先只需要blog名称信息，不需要它下面所有posts评论信息
        Thread.sleep(5000);
        System.out.println(resultMap.getPosts().get(0).getPostName()); //到这才需要BlogPostsResultMap.posts信息，
                                                                       // 此时才查询posts表。
    }
